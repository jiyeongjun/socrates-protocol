...
test("Claude stop hook continues when clarifying context is relevant", async () => {
...
test("Claude stop hook stays silent when already continued once", async () => {
...
test("Claude stop hook stays silent when the assistant already asked the next question", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "socrates-claude-stop-asked-"));
  await writeFile(path.join(root, "SOCRATES_CONTEXT.md"), buildDoc(), "utf8");
...
test("Claude stop hook stays silent for unrelated assistant messages", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "socrates-claude-stop-unrelated-"));
  await writeFile(path.join(root, "SOCRATES_CONTEXT.md"), buildDoc(), "utf8");
