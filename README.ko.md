...
아래 예시는 현재 태그 버전인 `v0.3.1`에 고정됩니다.
현재 package version은 `0.3.1`입니다.
...
- hook은 스킬 스코프가 아니라 설정 스코프이므로, 오래된 clarifying 파일이 남아 있으면 같은 repo의 비-Socrates 작업에도 영향을 줄 수 있습니다
- 포함된 구현은 이제 엄격한 명시적 상태 기준으로 동작합니다. canonical `SOCRATES_CONTEXT.md`, `status: "clarifying"`, `clarifying_phase: "needs_question"`, 남아 있는 `next_question`이 있어야만 개입합니다
- 마지막 assistant 메시지를 해석해서 질문을 “충분히 이미 물었는지” 추측하지 않습니다
- Stop hook이 빠지려면 공유 맥락 파일이 더 이상 `clarifying_phase: "needs_question"` 상태가 아니어야 합니다. 실제로는 질문 후 `awaiting_user_answer`로 다시 써야 합니다
- 오래된 clarifying 파일이 repo에 남아 있으면, `SOCRATES_CONTEXT.md`를 갱신·교체·삭제할 때까지 Stop hook이 계속 개입합니다
...
- hook은 스킬 스코프가 아니라 설정 스코프이므로, 오래된 clarifying 파일이 남아 있으면 같은 프로젝트의 비-Socrates 작업에도 영향을 줄 수 있습니다
- 포함된 구현은 이제 엄격한 명시적 상태 기준으로 동작합니다. canonical `SOCRATES_CONTEXT.md`, `status: "clarifying"`, `clarifying_phase: "needs_question"`, 남아 있는 `next_question`이 있어야만 개입합니다
- 마지막 assistant 메시지를 해석해서 질문을 “충분히 이미 물었는지” 추측하지 않습니다
- Stop hook이 빠지려면 공유 맥락 파일이 더 이상 `clarifying_phase: "needs_question"` 상태가 아니어야 합니다. 실제로는 질문 후 `awaiting_user_answer`로 다시 써야 합니다
- 오래된 clarifying 파일이 프로젝트에 남아 있으면, `SOCRATES_CONTEXT.md`를 갱신·교체·삭제할 때까지 Stop hook이 계속 개입합니다
...
현재 태그 버전은 `v0.3.1`입니다.
현재 package version은 `0.3.1`입니다.

- 빠른 설치 예시는 재현 가능한 동작을 위해 현재 태그에 고정합니다
- `npm run verify:release-assets` 는 기본적으로 현재 worktree를 검사합니다
- 새 태그를 배포하기 전에는 `npm run verify:release-assets -- --ref v0.3.1` 로 installer가 요구하는 asset이 그 release ref에 모두 들어 있는지 확인하세요
- `0.x` 릴리스는 minor 버전 사이에서도 계약이 바뀔 수 있는 불안정한 단계로 보세요

### 맥락 파일 포맷

`SOCRATES_CONTEXT.md`의 YAML frontmatter는 현재 `version: 2`를 사용합니다.
...
- `version: 2`만 canonical runtime 포맷으로 신뢰합니다. legacy `version: 1` 파일은 hook이 다시 믿기 전에 normalize하거나 삭제해야 합니다.
- 파일이 손상됐거나 canonical body 섹션이 frontmatter와 어긋나면 canonical version 2 형식으로 정리할지 먼저 묻습니다.
