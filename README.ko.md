...
```text
$socrates 필수 아티팩트가 없으면 먼저 코드베이스에서 찾을 수 있는지 확인하고, 찾을 수 없을 때만 물어봐. 고위험 작업이면 가장 안전에 큰 영향을 주는 질문부터 먼저 해줘. 이름 변경이 호환성 경계를 넘는다면 mechanical rename으로 처리하지 말고 마이그레이션 전략부터 확인해줘. 구현 경로가 이미 명확하거나 하나가 확실히 우세하면 바로 구현해줘.
```
...
```bash
$socrates "코드가 엉망이에요. 우아하게 만들어줘"
# Socrates는 "우아하게"를 구현 기준으로 바로 받아들이지 않습니다.
# 코드를 바꾸기 전에 "우아하게"가 무엇을 뜻하는지 먼저 묻습니다.
```
