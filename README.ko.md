...
아래 예시는 현재 태그 버전인 `v0.2.2`에 고정됩니다.
...
- 위 quick install 명령은 스킬 파일과 Socrates `SessionStart` hook을 함께 설치하고, 기존 `hooks.json`과 병합합니다
...
- 위 권장 install 명령은 Socrates 스킬과 보수적인 `SessionStart` hook을 함께 설치합니다
...
- 탐색은 가장 가까운 git root까지만 올라가므로, nested repo가 상위 repo의 `SOCRATES_CONTEXT.md`를 잘못 집는 일은 막습니다
- 위 quick install 명령은 스킬 파일과 Socrates `SessionStart` hook을 함께 설치하고, 기존 `.claude/settings.json`과 병합합니다
