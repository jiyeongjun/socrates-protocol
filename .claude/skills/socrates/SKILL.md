...
Compatibility-sensitive renames are not on the fast path until the migration strategy (hard cutover, compatibility transition, or multi-step migration) is explicit or one compatibility policy is clearly dominant from the stated constraints.
Signals that a rename may cross a compatibility boundary: UPPER_SNAKE_CASE names, names in env or config files, package entry point exports, persisted schema fields, migration-referenced identifiers, or external contract names.

Branches are materially different when the user would reject one implementation in favor of the other — i.e., they differ in infrastructure dependencies, data flow, observable behavior, or user-facing contract.
...
For rename requests, also validate whether the rename crosses a compatibility boundary. If it does, decide whether the change is a hard cutover, a compatibility transition, or a multi-step migration (e.g. expand-contract for persisted fields) before implementing.
...
- Do not treat every rename as mechanical. If a rename crosses a compatibility boundary, ask one compatibility question first unless the migration strategy is already stated or clearly implied.
...
- When an undefined preference word and high-risk signals co-occur, use the high-risk response pattern (1 to 3 questions). If a single question can operationalize the preference and resolve the safety constraint together, prefer that.
...
- Compatibility-sensitive rename: ask one migration-strategy question (hard cutover, compatibility transition, or multi-step migration) first unless the rename policy is already explicit, then stop.
...
- If the user says `Rename the customer_id column to account_id`, ask whether this is a single-cutover migration or an expand-contract (add new column, backfill, migrate readers, drop old) if the migration strategy is not already stated.
