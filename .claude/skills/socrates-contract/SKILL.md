...
6. If the goal requires multiple problems, multiple turns, protected-surface planning, or durable handoff, create visible contract files instead of one large context file. Threshold: a single file, single verification path, and at most one reversible decision can stay inline; anything broader needs `contract-index.md`. Use `contract-index.md` at the workspace root and `contracts/contract-001.md`, `contracts/contract-002.md`, etc. See [references/contract-files.md](references/contract-files.md).
...
8. Execute one active subcontract at a time. Mutate only `contract-index.md`, the active subcontract file, and the direct implementation artifacts needed for that subcontract. Read-only exploration of related code or documents is allowed and encouraged.
